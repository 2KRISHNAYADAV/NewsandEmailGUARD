    ...
    def clean_text(self, text):
        ...
        for token in tokens:
            if token not in self.stop_words and len(token) > 2:
                # Apply stemming for better accuracy
...
def load_email_data():
    ...
    for filename in os.listdir(spam_dir):
        if filename.endswith('.txt') or '.' in filename:
            ...
            content = ImprovedEmailPreprocessor().extract_email_content(file_path)
            if content.strip():
                email_data.append({
                    'content': content,
                    'label': 1  # Spam
...
def train_and_evaluate_improved_models(X_train, X_test, y_train, y_test, dataset_name):
    """Train and evaluate improved models for better accuracy"""
    print(f"\n=== Training improved models for {dataset_name} ===")
    
    # Improved TF-IDF Vectorization
    print("Vectorizing text data with improved features...")
    ...
    # Save vectorizer
    joblib.dump(tfidf, f'models/{dataset_name}_improved_tfidf_vectorizer.pkl')
    
    # Improved models with optimized parameters
    models = {
    ...
    for name, model in models.items():
        ...
        # Save model
        joblib.dump(model, f'models/{dataset_name}_improved_{name.lower().replace(" ", "")}.pkl')
        ...
        plt.savefig(f'models/{dataset_name}_improved_{name.lower().replace(" ", "")}_confusion_matrix.png')
        plt.close()
    ...
    # Save ensemble model
    joblib.dump(ensemble, f'models/{dataset_name}_improved_ensemble.pkl')
    
    return results, tfidf

def main():
    """Main function to run the improved training pipeline"""
    print("=== Improved NLP Fake News and Spam Email Classifier ===")
    ...
    news_preprocessor = ImprovedTextPreprocessor()
    ...
    email_preprocessor = ImprovedEmailPreprocessor()
    ...
    # 3. Train and evaluate improved news models
    news_results, news_tfidf = train_and_evaluate_improved_models(
        X_news_train, X_news_test, y_news_train, y_news_test, 'news'
    )
    
    # 4. Train and evaluate improved email models
    email_results, email_tfidf = train_and_evaluate_improved_models(
        X_email_train, X_email_test, y_email_train, y_email_test, 'email'
    )
    
    # 5. Save improved preprocessors
    joblib.dump(news_preprocessor, 'models/news_improved_preprocessor.pkl')
    joblib.dump(email_preprocessor, 'models/email_improved_preprocessor.pkl')
    
    # 6. Save cleaned data
    news_df.to_csv('data/improved_cleaned_news_data.csv', index=False)
    email_df.to_csv('data/improved_cleaned_email_data.csv', index=False)
    
    print("\n=== Improved Training Complete ===")
    print("Improved models saved in 'models/' directory")
    print("Improved cleaned data saved in 'data/' directory")
